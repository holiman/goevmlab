...
// The BorBatchVM spins up one 'master' instance of the VM, and uses that to execute tests
type BorBatchVM struct {
	BorEVM
...
func NewBorBatchVM(path, name string) *BorBatchVM {
	return &BorBatchVM{
		BorEVM: BorEVM{path, name, &VmStat{}},
	}
}

func (evm *BorBatchVM) Instance(threadId int) Evm {
	return &BorBatchVM{
		BorEVM: BorEVM{
			path:  evm.path,
...
func (evm *BorBatchVM) RunStateTest(path string, out io.Writer, speedTest bool) (*tracingResult, error) {
	var (
		t0     = time.Now()
		err    error
		cmd    *exec.Cmd
...
func (evm *BorBatchVM) GetStateRoot(path string) (root, command string, err error) {
	if evm.cmd == nil {
