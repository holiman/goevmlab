...
// BorEVM is s Evm-interface wrapper around the bor `evm` binary, based on go-ethereum.
type BorEVM struct {
	path string
...
func NewBorEVM(path string, name string) *BorEVM {
	return &BorEVM{
		path:  path,
		name:  name,
		stats: &VmStat{},
...
func (evm *BorEVM) Instance(int) Evm {
	return evm
}

func (evm *BorEVM) Name() string {
	return evm.name
}
...
func (evm *BorEVM) GetStateRoot(path string) (root, command string, err error) {
...
func (evm *BorEVM) ParseStateRoot(data []byte) (string, error) {
...
func (evm *BorEVM) RunStateTest(path string, out io.Writer, speedTest bool) (*tracingResult, error) {
	var (
		t0     = time.Now()
