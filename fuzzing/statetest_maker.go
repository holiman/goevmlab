...
func CreatePayload() []byte {
	base, _ := new(big.Int).SetString("980558841592023691157015463309422336774674025094029443179783308536337341468023231207790742816000618158721929594940766299", 10)
	exp, _ := new(big.Int).SetString("27862535351960703627033598774", 10)
	mod, _ := new(big.Int).SetString("", 10)

	p := program.NewProgram()

	lengths := make([]byte, 96)
	copy(lengths[0:], common.LeftPadBytes([]byte{50}, 32))
	copy(lengths[32:], common.LeftPadBytes([]byte{12}, 32))
	copy(lengths[64:], common.LeftPadBytes([]byte{3, 124}, 32))
	// 50 12 892
	var data []byte
	data = append(data, lengths...)
	data = append(data, base.Bytes()...)
	data = append(data, exp.Bytes()...)
	data = append(data, mod.Bytes()...)
	p.Mstore(data, 0)
	p.StaticCall(big.NewInt(1500000), common.HexToAddress("0x5"), 0, len(data), 0, 64)
	return p.Bytecode()
}
